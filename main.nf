...
process hostremoval_input_fastq {
    ...
    if ( seqtype == 'SE' ) {
        ...
        """
        samtools index $bam
        extract_map_reads.py $bam ${r1} -m ${params.hostremoval_mode} $merged -of $out_fwd -p ${task.cpus} 
        """
    } else {
        ...
        """
        samtools index $bam
        extract_map_reads.py $bam ${r1} -rev ${r2} -m ${params.hostremoval_mode} $merged -of $out_fwd -or $out_rev -p ${task.cpus}
        """ 
    }
