...
process lanemerge_stripfastq {
  ...
  input:
  tuple samplename, libraryid, lane, colour, seqtype, organism, strandedness, udg, path(r1), path(r2) from ch_input_for_lanemerge_stripfastq.groupTuple(by: [0,1,3,4,5,6,7])
...
process fastqc_after_clipping {
    ...
    input:
    tuple samplename, libraryid, lane, seqtype, organism, strandedness, udg, path(r1), path(r2) from ch_adapterremoval_for_fastqc_after_clipping
...
process bwamem {
    ...
    input:
    tuple samplename, libraryid, lane, seqtype, organism, strandedness, udg, path(r1), path(r2) from ch_lanemerge_for_bwamem
...
process bowtie2 {
    ...
    input:
    tuple samplename, libraryid, lane, seqtype, organism, strandedness, udg, path(r1), path(r2) from ch_lanemerge_for_bt2
...
process dedup{
    ...
    tuple samplename, libraryid, lane, seqtype, organism, strandedness, udg, path("${libraryid}_rmdup.bam"), path("*.{bai,csi}") into ch_output_from_dedup
...
process markDup{
    ...
    tuple samplename, libraryid, lane, seqtype, organism, strandedness, udg, path("${libraryid}_rmdup.bam"), path("*.{bai,csi}") into ch_output_from_markdup
...
if (params.additional_vcf_files == '') {
 ...
 process multivcfanalyzer {
  ...
  path('MultiVFAnalyzer.json') optional true into ch_multivcfanalyzer_for_multiqc
...
process kraken {
  ...
  input:
  path(fastq) from ch_bam_filtering_for_metagenomic_kraken.map { it[7] }.collect()
...
process kraken_merge {
  ...
  input:
  tuple val(name), path(csv_count) from ch_kraken_parsed.collect()
...
process multiqc {
    ...
    path('fastqc_raw/*') from ch_prefastqc_for_multiqc.collect().ifEmpty([])
    path('fastqc/*') from ch_fastqc_after_clipping.collect().ifEmpty([])
    path(software_versions_mqc) from software_versions_yaml.collect().ifEmpty([])
    path('adapter_removal/*') from ch_adapterremoval_logs.collect().ifEmpty([])
    path('mapping/bt2/*') from ch_bt2_for_multiqc.collect().ifEmpty([])
    path('flagstat/*') from ch_flagstat_for_multiqc.collect().ifEmpty([])
    path('flagstat_filtered/*') from ch_bam_filtered_flagstat_for_multiqc.collect().ifEmpty([])
    path('preseq/*') from ch_preseq_for_multiqc.collect().ifEmpty([])
    path('damageprofiler/dmgprof*/*') from ch_damageprofiler_results.collect().ifEmpty([])
    path('qualimap/qualimap*/*') from ch_qualimap_results.collect().ifEmpty([])
    path('markdup/*') from ch_markdup_results_for_multiqc.collect().ifEmpty([])
    path('dedup*/*') from ch_dedup_results_for_multiqc.collect().ifEmpty([])
    path('fastp/*') from ch_fastp_for_multiqc.collect().ifEmpty([])
    path('sexdeterrmine/*') from ch_sexdet_for_multiqc.collect().ifEmpty([])
    path('mutnucratio/*') from ch_mtnucratio_for_multiqc.collect().ifEmpty([])
    path('endorspy/*') from ch_endorspy_for_multiqc.collect().ifEmpty([])
    path('multivcfanalyzer/*') from ch_multivcfanalyzer_for_multiqc.collect().ifEmpty([])
    path('malt/*') from ch_malt_for_multiqc.collect().ifEmpty([])
    path('kraken/*') from ch_kraken_for_multiqc.collect().ifEmpty([])
    path('hops/*') from ch_hops_for_multiqc.collect().ifEmpty([])
    path(logo) from ch_eager_logo
