...
Depending what is supplied pipeline options by the user, and if `--save_reference` is supplied, this directory will contain various reference index files based on the FASTA file given to `--fasta`.

It is highly recommend to move these files to a central location or cache directory on your machine to facilitate resume of the indices across different pipeline runs. In many cases indexing the reference genome for alignment can be the longest step of a pipeline run, therefore re-using indices in future runs (supplied pipeline flags such as `--fasta_fai` or `--fasta_dict` etc.) can greatly speed up analyses on other samples.
