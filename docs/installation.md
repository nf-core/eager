# nf-core/eager: Installation

To start using the nf-core/eager pipeline, follow the steps below:
...
```bash
wget https://github.com/nf-core/eager/archive/master.zip
mkdir -p ~/my-pipelines/nf-core/
unzip master.zip -d ~/my-pipelines/nf-core/
cd ~/my_data/
nextflow run ~/my-pipelines/nf-core/eager-master
```
...
```bash
nextflow run /path/to/nf-core-eager -with-singularity nf-core-eager.simg
```
