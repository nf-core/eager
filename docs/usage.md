...
Providing a reference sheet to `--fasta` allows users to align their input reads to _multiple_ reference genomes in the same run. The reference sheet must be in the format of a table, and the file extension must be either `csv` or `tsv`.
...
Note that passing a reference sheet to the pipeline with `--fasta` will _override_ any corresponding directly-supplied parameters specifying user-build indices (`--fasta_fai`, `--fasta_dict`).
...
| mitochondrion    | No       | Optional string, and only required for MTNucRatio, with name of entry in FASTA of mitochondrion entry's header.            |

Files for `fai`, `dict`, `mapper_index` will be generated by the pipeline for you if not specified.

A real-world example could look as follows, where a user-supplied `.dict` file nor circular generator not MtNucRatio is required:
