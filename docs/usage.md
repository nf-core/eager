...
Providing a reference sheet to `--fasta` allows users to simultaneously align their input reads (separately) to _multiple_ reference genomes. The reference sheet must be in the format of a table, and the file extension to be either `csv` or `tsv`.

In addition to including the path to the FASTA, the reference sheet is also used to specify paths to optional pre--built indices of each reference (namely, `.fai` from `samtools faidx`, `.dict` from `picard CreateSequenceDictionary`, and a directory pointing to a directory containing the indices for the given mapper - e.g. `bwa index`).

Note that passing the reference sheet to `--fasta` to the pipeline will _override_ the corresponding user-build indices parameters (`--fasta_fai`, `--fasta_dict`).
...
Only `reference_name`, and `fasta` are mandatory, whereas all other entries can be missing depending on your context.
...
| mitocondrion    | No       | Optional string, and only required for MTNucRatio, with name of entry in FASTA of mitochondrion entry's header.            |
...
> ⚠️ The names of the files in the `mapper_index` directory must have the same basename as the FASTA itself! e.g. if FASTA is Mammoth_MT_Krause.fasta, then each BWA index file should be Mammoth_MT_Krausef.fasta.\<suffix\>
...
```bash
nextflow run nf-core/eager --input samplesheet.csv --outdir <OUTDIR> --genome GRCh37 -profile docker
```
